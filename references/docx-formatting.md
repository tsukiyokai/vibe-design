...
本文档定义了设计文档转 .docx 时的完整排版规范。使用 docx-js (`npm install -g docx`) 生成。
...
| 页边距 | 上下 1440 DXA (1 英寸)，左右 1440 DXA |
| 正文区宽度 | 9026 DXA |
| 页眉 | 右对齐，灰色小字：项目名 + "设计文档"（如 "XXX 软件设计文档"） |
| 页脚 | 居中显示页码 |

## 目录页

文档第一页为目录页（不生成封面页），使用 `TableOfContents`，需配合 heading styles 的 `outlineLevel`：
...
采用带编号的层级标题，通过 docx-js 的 numbering 配置实现自动编号：
...
|------|------|
| 对齐 | 居中 |
| 最大宽度 | 正文区的 90% (约 560px / 8123 DXA) |
| 段前/段后 | 200 DXA |
| 图注 | 图片下方居中，灰色 9pt，格式："图 N: <标题>" |
...
生成的 docx 文件不含封面页，只有一个 section（含页眉页脚），标题层级严格匹配所选模板：
